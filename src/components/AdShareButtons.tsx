...
import { Twitter, Facebook, Instagram, Mail, Send, Loader2 } from "lucide-react";
...
const AdShareButtons = ({ adName, tweetText, className = "" }: AdShareButtonsProps) => {
  ...
  const postToTwitter = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('post-tweet', {
        body: { tweet: truncatedTweet }
      });

      if (error) throw error;
      
      if (data.success) {
        toast.success(`${adName} posted to Twitter!`);
      } else {
  ...
  const postToInstagram = () => {
    // Instagram doesn't have a direct share API, so we copy the text and inform user
